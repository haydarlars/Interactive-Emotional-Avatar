...
class LMStudioService {
  constructor() {
    this.endpoint = import.meta.env.VITE_LM_STUDIO_URL;
    this.model = import.meta.env.VITE_LM_STUDIO_MODEL;
    this.apiKey = import.meta.env.VITE_LM_STUDIO_API_KEY;
  }

  /**
   * Send a question to LM Studio and get an answer
  ...
  async askQuestion(question) {
    ...
    if (!this.apiKey) {
      throw new Error('LM Studio API key not set in environment variables.');
    }
    ...
    try {
      ...
      if (!response.ok) throw new Error(`LM Studio request failed: ${response.statusText}`);

      const data = await response.json();
      const answer = data?.choices?.[0]?.message?.content;
      if (!answer) throw new Error('No response from LM Studio');
      return answer;
    } catch (error) {
      console.error('LM Studio askQuestion error:', error);
      throw error;
    }
  }
...
Choose the animation and AI_emotion that best matches your response tone and the user's situation.`;
    ...
    try {
      ...
      try {
        ...
        // Fallback if LM Studio returns text instead of JSON
      ...
      console.error('LM Studio respondAsFriend error:', error);
      throw error;
    }
  }
}
